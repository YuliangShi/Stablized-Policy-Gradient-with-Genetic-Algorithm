...
def train_PGGA(
    ...
    for i in range(n_iter):
        ...
        new_obj, loss, gradients = train_PG(net, net_optim,
                                      env, max_path_length,
        ...
        if count == 2 or imp < - abs(imp_ave) * 5:
        # if True:
            count = 0
            print("*****************************************")
            performance = new_obj
...
def main():
    ...
    parser.add_argument('--exp_name', type=list, default=['PGGA'], nargs="*")
    ...
    parser.add_argument('--n_experiments', '-e', type=int, default=5)

    parser.add_argument('--learning_rate', '-lr', type=float, default=1e-2)
    parser.add_argument('--batch_size', '-b', type=int, default=2000)
    parser.add_argument('--gamma', '-gm', type=float, default=0.9)

    parser.add_argument('--n_gen', '-g', type=int, default=2)
    parser.add_argument('--ave_weight', '-aw', type=float, default=1)
    parser.add_argument('--sigma', '-si', type=float, default=100.)
    parser.add_argument('--num_workers', '-w', type=int, default=50)
    ...
    name = args.exp_name[0] + '_' + args.env_name + "_Net" + str(args.n_layers) + "_" + str(args.size) + \
        "_pop" + str(args.num_workers) + \
        "_g" + str(args.n_gen) + \
        "_lr" + str(args.learning_rate) + \
        "_bz" + str(args.batch_size) + \
        "_si" + str(args.sigma) + \
        "_gm" + str(args.gamma) + \
        "_aw" + str(args.ave_weight) + \
        "_ml" + str(args.max_trjc_len) + \
        "_T" + str(args.T) + \
        "_s" + str(args.seed)
